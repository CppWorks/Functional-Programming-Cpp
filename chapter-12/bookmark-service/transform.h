...
namespace reactive {
  ...
  namespace detail {
    template <typename Sender, typename Transformation,
              ...
              typename MessageType = decltype(
                std::declval<Transformation>()(std::declval<SourceMessageType>()))>
  ...
  namespace operators {
    ...
    {
      return detail::transform_helper<Transformation>{ std::forward<Transformation>(
        transformation) };
    }
